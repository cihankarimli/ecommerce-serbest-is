...
function createBasketItem(product) {
  ...
  //basketdeki elemmentin artilib azalmasi
...
function updateProductQuantity(product, number) {
  let index = products.findIndex(
    (p) => p.imgSrc === product.imgSrc && p.cardTitle === product.cardTitle
  );
  if (index !== -1) {
...
//basketden elementi silmek sebetden silinende localstorage-dan silinmesi
...
for (let i = 0; i < basketButton.length; i++) {
  basketButton[i].addEventListener("click", function () {
    ...
    let existingProduct = products.find(
      (p) => p.imgSrc === imgSrc && p.cardTitle === cardTitle
    );
